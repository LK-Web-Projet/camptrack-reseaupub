...
interface EditAppelModalProps {
    isOpen: boolean;
    onClose: () => void;
    appel: Appel;
    onEditAppel?: (updated: Appel) => void;
}
...
export default function EditAppelModal({ isOpen, onClose, appel, onEditAppel }: EditAppelModalProps) {
    ...
    const formik = useFormik({
        ...
        initialValues: {
            id_prestataire: appel.id_prestataire,
            date_appel: toLocalDatetimeValue(appel.date_appel),
            direction: appel.direction,
            motif: appel.motif,
            duree_minutes: appel.duree_minutes != null ? String(appel.duree_minutes) : "",
        ...
        onSubmit: async (values) => {
            try {
                ...
                const res = await apiClient(`/api/appels/${appel.id_appel}`, {
                    method: "PATCH",
                ...
                const updated: Appel = await res.json();
                toast.success("Appel modifié avec succès");
                if (onEditAppel) onEditAppel(updated);
                onClose();
            } catch (err) {
                toast.error(err instanceof Error ? err.message : "Erreur lors de la modification de l'appel");
            }
        },
    });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-lg bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-6 max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-semibold text-[#d61353]">Modifier l&apos;appel</h2>
